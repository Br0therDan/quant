...
from shared.models import Signal, Position
...
class StrategyConfig(BaseModel):
    ...
    # 공통 설정
    lookback_period: int = Field(252, description="백테스트 룩백 기간 (일)")
    min_data_points: int = Field(30, description="최소 필요 데이터 포인트")
    
    # 리스크 관리
    max_position_size: float = Field(1.0, description="최대 포지션 크기 (비율)")
    stop_loss_pct: Optional[float] = Field(None, description="손절 비율")
    take_profit_pct: Optional[float] = Field(None, description="익절 비율")
...
class BaseStrategy(ABC):
    ...
    def run(self, data: pd.DataFrame) -> List[StrategySignal]:
        ...
        if not self.validate_data(data):
            raise ValueError(f"데이터가 유효하지 않습니다. 최소 {self.config.min_data_points}개 데이터 포인트가 필요합니다.")
