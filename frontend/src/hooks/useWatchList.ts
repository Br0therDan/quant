...
export const watchlistQueryKeys = {
	...
	watchlistCoverage: (name: string) =>
		[...watchlistQueryKeys.coverage(), name] as const,
};
...
export function useWatchlist() {
	...
	const createWatchlistMutation = useMutation({
		...
		onSuccess: (_, variables) => {
			...
			showSuccess(
				`워치리스트 "${variables?.name || "새 워치리스트"}"가 성공적으로 생성되었습니다`,
			);
		},
		onError: (error) => {
		...
		onSuccess: (_, variables) => {
			...
			if (variables?.name) {
				queryClient.invalidateQueries({
					queryKey: watchlistQueryKeys.detail(variables.name),
				});
			}
			showSuccess(
				`워치리스트 "${variables?.name || "default"}"가 성공적으로 저장되었습니다`,
			);
		},
		onError: (error) => {
	...
	const updateWatchlistMutation = useMutation({
		...
		onSuccess: (_, variables) => {
			...
			showSuccess(
				`워치리스트 "${variables.name}"가 성공적으로 업데이트되었습니다`,
			);
		},
		onError: (error, variables) => {
			console.error("워치리스트 업데이트 실패:", error);
			showError(
				`워치리스트 "${variables.name}" 업데이트에 실패했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
			);
		},
	});
	...
	const deleteWatchlistMutation = useMutation({
		...
		onError: (error, name) => {
			...
			showError(
				`워치리스트 "${name}" 삭제에 실패했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
			);
		},
	});
	...
	const setupDefaultWatchlistMutation = useMutation({
		...
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: watchlistQueryKeys.lists() });
			queryClient.invalidateQueries({
				queryKey: watchlistQueryKeys.detail("default"),
			});
