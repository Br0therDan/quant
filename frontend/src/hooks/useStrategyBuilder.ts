...
export const useStrategyBuilder = () => {
    ...
    const generateStrategyMutation = useMutation({
        ...
        onSuccess: (data, query) => {
            ...
            const assistantMessage: Message = {
                ...
                metadata: {
                    ...
                    indicators: data.generated_strategy?.indicators?.map(i => i.indicator_name),
                },
            };
        ...
        onError: (error) => {
            showError(`전략 생성 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
        },
    });
    ...
    const searchIndicatorsMutation = useMutation({
        ...
        onSuccess: (data) => {
            if (data?.indicators) {
                setRecommendations(data.indicators as unknown as IndicatorRecommendation[]);
            }
        },
        onError: (error) => {
            showError(`지표 검색 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
        },
    });
    ...
    const approveStrategyMutation = useMutation({
        ...
        onError: (error) => {
            showError(`승인 처리 실패: ${error instanceof Error ? error.message : '알 수 없는 오류'}`);
        },
    });
    ...
    const generateResponseMessage = (response: StrategyBuilderResponse): string => {
        const parts: string[] = [];

        // 의도 파싱 결과
        if (response.parsed_intent) {
            const intent = response.parsed_intent;
            parts.push(`💡 **의도**: ${intent.intent_type} (신뢰도: ${(intent.confidence * 100).toFixed(0)}%)`);
        ...
        if (response.generated_strategy) {
            ...
            if (strategy.indicators && strategy.indicators.length > 0) {
                const indicators = strategy.indicators.map(i => i.indicator_name).join(', ');
        ...
        if (response.human_approval?.requires_approval) {
            parts.push(`\n⚠️ **승인 필요**`);
            if (response.human_approval.approval_reasons && response.human_approval.approval_reasons.length > 0) {
                parts.push(`사유: ${response.human_approval.approval_reasons.join(', ')}`);
            }

            if (response.human_approval.suggested_modifications) {
                const mods = Object.entries(response.human_approval.suggested_modifications)
                    .map(([key, value]) => `${key}: ${value}`)
                    .join(', ');
    ...
    const sendMessage = useCallback((message: string) => {
        ...
        if (message.toLowerCase().includes('지표') || message.toLowerCase().includes('indicator')) {
    ...
    return {
        ...
        approveStrategy: (request: StrategyApprovalRequest) => approveStrategyMutation.mutate(request),
        clearConversation,

        // 편의 함수
        parseIntent: () => conversation.currentStrategy?.parsed_intent,
        getGeneratedStrategy: () => conversation.currentStrategy?.generated_strategy,
