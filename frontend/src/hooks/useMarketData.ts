...
export const marketDataQueryKeys = {
	...
	coverageSymbol: (symbol: string) =>
		[...marketDataQueryKeys.coverage(), symbol] as const,
...
export function useMarketData() {
	...
	const collectCompanyInfoMutation = useMutation({
		...
		onError: (error, symbol) => {
			...
			showError(
				`${symbol} 기업 정보 수집에 실패했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
			);
		},
	});
	...
	const collectMarketDataMutation = useMutation({
		...
		onError: (error, symbol) => {
			...
			showError(
				`${symbol} 주가 데이터 수집에 실패했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
			);
		},
	});
	...
	const collectBulkDataMutation = useMutation({
		...
		onSuccess: (_, symbols) => {
			queryClient.invalidateQueries({
				queryKey: marketDataQueryKeys.coverage(),
			});
			queryClient.invalidateQueries({
				queryKey: marketDataQueryKeys.systemStatus(),
			});
		...
		onError: (error) => {
			...
			showError(
				`대량 데이터 수집에 실패했습니다: ${error instanceof Error ? error.message : "알 수 없는 오류"}`,
			);
		},
	});
