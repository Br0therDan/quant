...
export interface Dataset {
    ...
    correlation_matrix?: { feature1: string; feature2: string; correlation: number }[];
}
...
export const useFeatureStore = (params?: FeaturesQueryParams) => {
    ...
    const createFeatureMutation = useMutation({
        ...
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: featureStoreQueryKeys.features() });
    ...
    const updateFeatureMutation = useMutation({
        mutationFn: async ({ featureId, data }: { featureId: string; data: FeatureUpdate }): Promise<Feature> => {
            // TODO: Replace with actual API call
    ...
    const featureDetailQuery = useQuery({
        ...
        queryFn: async (): Promise<FeatureDetail> => {
            ...
            return {
                ...
                transformation_code: "df['new_feature'] = df['price'].rolling(20).mean()",
                dependencies: ["price"],
            };
        },
        enabled: !!featureId,
