...
import { TemplateService } from "@/client";
...
export function useTemplates() {
    ...
    const createTemplateMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: templateQueryKeys.usageStats() });
            showSuccess(`템플릿 "${data?.name || '새 템플릿'}"이 성공적으로 생성되었습니다`);
        },
        onError: (error) => {
            console.error("템플릿 생성 실패:", error);
            showError(error instanceof Error ? error.message : '템플릿 생성에 실패했습니다');
        },
    });
    ...
    const updateTemplateMutation = useMutation({
        ...
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: templateQueryKeys.lists() });
            queryClient.invalidateQueries({ queryKey: templateQueryKeys.detail(data?.id || '') });
            showSuccess(`템플릿 "${data?.name || '템플릿'}"이 성공적으로 업데이트되었습니다`);
        },
        onError: (error) => {
            console.error("템플릿 업데이트 실패:", error);
            showError(error instanceof Error ? error.message : '템플릿 업데이트에 실패했습니다');
        },
    });
    ...
    const deleteTemplateMutation = useMutation({
        ...
        onError: (error) => {
            ...
            showError(error instanceof Error ? error.message : '템플릿 삭제에 실패했습니다');
        },
    });
    ...
    const createStrategyFromTemplateMutation = useMutation({
        ...
        onSuccess: (data) => {
            ...
            queryClient.invalidateQueries({ queryKey: templateQueryKeys.usageStats() });
            showSuccess(`템플릿에서 전략 "${data?.name || '새 전략'}"이 성공적으로 생성되었습니다`);
        },
        onError: (error) => {
            console.error("템플릿에서 전략 생성 실패:", error);
            showError(error instanceof Error ? error.message : '템플릿에서 전략 생성에 실패했습니다');
        },
    });
    ...
    return useMemo(() => ({
        ...
        isPending: {
            templates: templateListQuery.isPending,
            templateUsageStats: templateUsageStatsQuery.isPending,
        },
        error: {
        ...
        createStrategyFromTemplate: createStrategyFromTemplateMutation.mutateAsync,
        ...
        isMutating: {
            ...
            createStrategyFromTemplate: createStrategyFromTemplateMutation.isPending,
        },
    }), [
        templateListQuery,
