...
export const cryptoQueryKeys = {
	...
	bulkExchangeRatesPair: (fromCurrency: string, toCurrencies: string[]) =>
		[
	...
	weeklyPricesSymbol: (
		...
		const key = [...cryptoQueryKeys.weeklyPrices(), symbol, market] as const;
		const params: Record<string, string> = {};
		if (options?.startDate) params.startDate = options.startDate;
		if (options?.endDate) params.endDate = options.endDate;
		return Object.keys(params).length > 0 ? ([...key, params] as const) : key;
	...
	monthlyPricesSymbol: (
		...
		const key = [...cryptoQueryKeys.monthlyPrices(), symbol, market] as const;
		const params: Record<string, string> = {};
		if (options?.startDate) params.startDate = options.startDate;
		if (options?.endDate) params.endDate = options.endDate;
		return Object.keys(params).length > 0 ? ([...key, params] as const) : key;
...
export const useCryptoExchangeRate = (
	...
	const queryKey = cryptoQueryKeys.exchangeRatePair(fromCurrency, toCurrency);
...
export const useCryptoDailyPrices = (
	...
	return useQuery({
		...
		enabled:
			options?.enabled !== undefined ? options.enabled : !!(symbol && market),
