...
export const strategyQueryKeys = {
	...
	list: (filters: string) =>
		[...strategyQueryKeys.lists(), { filters }] as const,
	...
	strategyExecutions: (id: string) =>
		[...strategyQueryKeys.executions(), id] as const,
	performance: () => [...strategyQueryKeys.all, "performance"] as const,
	strategyPerformance: (id: string) =>
		[...strategyQueryKeys.performance(), id] as const,
};
...
export function useStrategy() {
	...
	const createStrategyMutation = useMutation({
		...
		onSuccess: (data) => {
			...
			showSuccess(
				`전략 "${data?.name || "새 전략"}"이 성공적으로 생성되었습니다`,
			);
		},
		onError: (error) => {
			console.error("전략 생성 실패:", error);
			showError(
				error instanceof Error ? error.message : "전략 생성에 실패했습니다",
			);
		},
	});
	...
	const updateStrategyMutation = useMutation({
		mutationFn: async (data: {
			id: string;
			updateData: Partial<StrategyCreate>;
		}) => {
		...
		onSuccess: (data) => {
			...
			showSuccess(
				`전략 "${data?.name || "전략"}"이 성공적으로 업데이트되었습니다`,
			);
		},
		onError: (error) => {
			console.error("전략 업데이트 실패:", error);
			showError(
				error instanceof Error ? error.message : "전략 업데이트에 실패했습니다",
			);
		},
	});
	...
	const executeStrategyMutation = useMutation({
		...
		onSuccess: (_, variables) => {
			queryClient.invalidateQueries({
				queryKey: strategyQueryKeys.strategyExecutions(variables.id),
			});
			queryClient.invalidateQueries({
				queryKey: strategyQueryKeys.strategyPerformance(variables.id),
			});
		...
		onError: (error) => {
			...
			showError(
				error instanceof Error ? error.message : "전략 실행에 실패했습니다",
			);
		},
	});
