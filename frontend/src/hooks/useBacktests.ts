...
export const backtestQueryKeys = {
	...
	list: (filters: string) =>
		[...backtestQueryKeys.lists(), { filters }] as const,
	...
	performanceState: () =>
		[...backtestQueryKeys.analytics(), "performanceState"] as const,
	tradeState: () => [...backtestQueryKeys.analytics(), "tradeState"] as const,
	summaryState: () =>
		[...backtestQueryKeys.analytics(), "summaryState"] as const,
...
export function useBacktest() {
	...
	const createBacktestMutation = useMutation({
		...
		onError: (error) => {
			...
			showError(
				error instanceof Error ? error.message : "백테스트 생성에 실패했습니다",
			);
		},
	});
		...
		onSuccess: (data) => {
			...
			if (data?.id) {
				queryClient.invalidateQueries({
					queryKey: backtestQueryKeys.detail(data.id),
				});
			}
			showSuccess(
	...
	const deleteBacktestMutation = useMutation({
		...
		onError: (error) => {
			...
			showError(
				error instanceof Error ? error.message : "백테스트 삭제에 실패했습니다",
			);
		},
	});
