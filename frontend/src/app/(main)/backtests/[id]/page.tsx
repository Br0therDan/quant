...
export default function BacktestDetailPage() {
		...
		() => [
			...
			},
			{
				...
				value: backtest?.config
					? `${backtestUtils.formatDate(
							backtest.config.start_date,
						)} ~ ${backtestUtils.formatDate(backtest.config.end_date)}`
					: "-",
			},
			{
				label: "심볼",
			...
			{
				label: "초기 자본",
				value: backtestUtils.formatCurrency(
					backtest?.config?.initial_cash ?? undefined,
				),
			},
			{
				label: "최대 포지션",
				value: backtestUtils.formatPercentage(
					backtest?.config?.max_position_size ?? undefined,
				),
			},
			{
				label: "수수료율",
				value: backtestUtils.formatPercentage(
					backtest?.config?.commission_rate ?? undefined,
					3,
				),
			},
			{
				label: "슬리피지",
				value: backtestUtils.formatPercentage(
					backtest?.config?.slippage_rate ?? undefined,
					3,
				),
			},
			{
	...
	if (detailLoading) {
		return (
			<PageContainer
				title="백테스트 상세"
				breadcrumbs={[{ title: "백테스트" }, { title: "상세" }]}
			>
				<Box sx={{ mt: 4 }}>
					<LinearProgress />
				</Box>
			...
			<Container maxWidth="lg">
				...
				<Paper>
										...
										<CardContent>
											...
											<Stack spacing={1}>
												...
												<Box>
													...
													<Typography variant="body1">
														{performance.winning_trades} 승 ·{" "}
														{performance.losing_trades} 패
													</Typography>
												</Box>
											</Stack>
										</CardContent>
					...
					<TabPanel value={activeTab} index={2}>
						<Alert severity="info">
							거래 내역 데이터는 아직 제공되지 않습니다. 백테스트 실행이
							완료되면 거래 내역이 표시됩니다.
						</Alert>
					</TabPanel>
				</Paper>
