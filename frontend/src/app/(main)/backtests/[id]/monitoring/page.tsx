...
export default function BacktestMonitoringPage() {
	...
	const { backtest, isLoading, refetch } = useBacktestDetail(backtestId);
		...
		>
			<Container maxWidth="lg">
				<Stack spacing={3} sx={{ mb: 4 }}>
						...
						<Stack spacing={1}>
							...
							<Stack direction="row" spacing={1} alignItems="center">
								...
								<Typography variant="body2" color="text.secondary">
									마지막 업데이트:{" "}
									{backtestUtils.formatDate(backtest.updated_at)}
								</Typography>
							</Stack>
						</Stack>
					...
					{backtestUtils.isRunning(backtest.status) ? (
						<Alert severity="info">
							백테스트가 실행 중입니다. 실시간 상태는 {refreshInterval / 1000}
							초마다 자동으로 업데이트됩니다.
						</Alert>
					) : (
							...
							<CardContent>
								<Stack spacing={1}>
									...
									<LinearProgress
										...
										value={
											backtestUtils.isRunning(backtest.status) ? undefined : 100
										}
									/>
