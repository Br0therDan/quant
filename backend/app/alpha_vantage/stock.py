...
class CoreStock(BaseAPIHandler):
    ...
    async def _call_core_stock_api(
        ...
        **kwargs: Any,
        ...
        return self._parse_core_stock_response(data, function, symbol, interval)
    ...
    def _parse_symbol_search(self, data: dict[str, Any]) -> dict[str, Any]:
        """
        심볼 검색 응답을 파싱합니다
        ...
        Returns:
            원시 Alpha Vantage 응답을 그대로 반환 (bestMatches 키 포함)
        """
        # Alpha Vantage 응답을 그대로 반환 (이미 올바른 형식)
        return data
    ...
    async def intraday(
        ...
        Args:
            ...
            month: 특정월의 과거데이터 쿼리 (기본값: None(최근거래일 일중데이터 반환), YYYY-MM 형식)
    ...
    async def daily(
        self,
        symbol: str,
        outputsize: Optional[Literal["compact", "full"]] = "compact",
    ) -> list[dict[str, Any]]:
        """
        일별 시계열 데이터를 가져옵니다
