...
class NarrativeReportService:
    ...
    def __init__(
        ...
        logger.info(f"NarrativeReportService initialized with model: {self.model}")
    ...
    async def generate_report(
        ...
        # 3. LLM 호출
        llm_output = await self._call_llm(context, language, detail_level)

        # 4. Pydantic 검증
        report = self._validate_output(llm_output, backtest_id)
