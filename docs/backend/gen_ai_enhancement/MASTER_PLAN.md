...
- [ ] T2.1.1: ChromaDB 의존성 추가 (`uv add chromadb`)
- [ ] T2.1.2: RAGService 클래스 생성
  - [ ] ChromaDB 클라이언트 초기화
  - [ ] 컬렉션 생성 (`user_backtests`, `user_strategies`)
- [ ] T2.1.3: `index_backtest_result()` 메서드
  - [ ] 백테스트 결과 → 텍스트 변환
  - [ ] OpenAI Embedding API 호출
  - [ ] ChromaDB 인덱싱
- [ ] T2.1.4: `search_similar_backtests()` 메서드
  - [ ] 쿼리 임베딩
  - [ ] 유사도 검색 (top_k=5)
- [ ] T2.1.5: `build_rag_prompt()` 메서드
  - [ ] 검색 결과 → 프롬프트 컨텍스트
...
- [ ] T2.2.1: StrategyBuilderService 통합
  - [ ] RAGService 주입
  - [ ] `generate_strategy_with_rag()` 메서드 추가
  - [ ] 유사 전략 검색 → 프롬프트 생성
- [ ] T2.2.2: ChatOpsAdvancedService 통합
  - [ ] RAGService 주입
  - [ ] 대화 컨텍스트에 RAG 결과 포함
- [ ] T2.2.3: BacktestService 이벤트 훅
  - [ ] 백테스트 완료 시 자동 인덱싱
  - [ ] `index_backtest_result()` 호출
- [ ] T2.2.4: ServiceFactory 통합
...
- [ ] T2.4.1: E2E 테스트 시나리오 작성
- [ ] T2.4.2: RAG 기반 전략 생성 테스트
- [ ] T2.4.3: RAG 기반 대화 테스트
- [ ] T2.4.4: 비용 절감 최종 검증
...
- **Phase 1**: 0% (0/4 sprints) - ⏳ **대기중**
- **Phase 2**: 0% (0/4 sprints) - ⏸️ **보류**
- **전체**: 0% (0/8 sprints)
...
| ------ | ------------------------ | --------- | ------ | ----------- |
| 1.1    | OpenAIClientManager 구현 | ⏳ 대기중 | 0%     | Day 2       |
| 1.2    | 기존 서비스 리팩토링     | ⏳ 대기중 | 0%     | Day 5       |
| 1.3    | 모델 선택 API 추가       | ⏳ 대기중 | 0%     | Day 7       |
| 1.4    | Phase 1 통합 테스트      | ⏳ 대기중 | 0%     | Day 8       |
| 2.1    | RAGService 구현          | ⏸️ 보류   | 0%     | Day 10      |
| 2.2    | RAG 서비스 통합          | ⏸️ 보류   | 0%     | Day 13      |
| 2.3    | RAG 품질 테스트          | ⏸️ 보류   | 0%     | Day 15      |
| 2.4    | Phase 2 통합 테스트      | ⏸️ 보류   | 0%     | Day 16      |
...
| Phase 2 시작         | 2025-10-30 | ⏸️ 보류   | RAGService 구현 시작          |
| Phase 2 완료         | 2025-11-12 | ⏸️ 보류   | RAG 통합 배포                 |
| 프로젝트 완료        | 2025-11-15 | ⏸️ 보류   | 최종 검증 및 문서화           |
...
| ChromaDB 성능 이슈         | 중간   | DuckDB 백엔드 사용, 인덱싱 배치 처리           | Backend | ⏸️ 보류   |
| 기존 서비스 회귀 버그      | 높음   | 리팩토링 전 전체 테스트 스위트 실행            | Backend | ⏳ 대기중 |
| RAG 응답 품질 저하         | 중간   | 유사도 임계값 튜닝, 프롬프트 엔지니어링        | Backend | ⏸️ 보류   |
| 프로젝트 일정 지연         | 중간   | Sprint 단위 진행률 체크, 우선순위 재조정       | PM      | ⏳ 대기중 |
| 의존성 충돌 (ChromaDB)     | 낮음   | 별도 가상환경 테스트, 의존성 버전 고정         | Backend | ⏸️ 보류   |
...
| RAG 컨텍스트 길이 초과       | 중간   | 상위 K개만 선택, 요약 프롬프트 추가    | Backend | ⏸️ 보류   |
| 임베딩 API 비용 증가         | 중간   | 캐싱 전략, 동일 텍스트 재임베딩 방지   | Backend | ⏸️ 보류   |
...
| RAG 검색 결과 개인정보 노출  | 중간   | user_id 필터링, 권한 검증              | Backend | ⏸️ 보류   |
...
**마지막 업데이트**: 2025-10-15  
**다음 리뷰**: 2025-10-22 (Phase 1 중간 점검)
