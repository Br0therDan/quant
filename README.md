...
Alpha Vantage 기반의 현대적인 마이크로서비스 아키텍처 퀀트 백테스트
플랫폼입니다. 데이터 수집, 전략 실행, 백테스트 분석, 포트폴리오 관리가 모두
통합된 엔터프라이즈급 백엔드 서비스입니다.
...
│   │   ├── core/              # 핵심 설정
│   │   ├── utils/             # 유틸리티
│   │   ├── data/              # DuckDB 데이터 파일 저장소
│   │   │   ├── README.md      # 데이터 디렉토리 설명
│   │   │   └── quant.duckdb   # DuckDB 메인 데이터베이스 (자동 생성)
...
- **하이브리드 데이터베이스**: MongoDB (메타데이터) + DuckDB (고성능 시계열
  캐시)
- **투명한 캐싱**: Alpha Vantage → DuckDB → MongoDB 자동 계층화
- **10-100배 성능 향상**: DuckDB 컬럼나 저장소로 밀리초 단위 쿼리
- **자동 데이터 수집**: 워치리스트 기반 자동 데이터 업데이트
- **데이터 품질**: 결측치 보간, 스플릿/배당 조정, 데이터 검증
- **ServiceFactory 패턴**: 통합 의존성 주입으로 서비스 간 일관성 보장
...
| **Cache**        | DuckDB 0.9+                 | 고성능 컬럼나 시계열 캐시 (10-100배 향상) |
| **Architecture** | ServiceFactory 패턴         | 의존성 주입, 서비스 생명주기 관리         |
...
DUCKDB_PATH=./backend/app/data/quant.duckdb    # DuckDB 파일 경로 (백엔드 앱 내부)
...
# 백엔드 서비스 상태 확인
curl http://localhost:8000/api/v1/backtests/health

# DuckDB 캐시 성능 통계
curl http://localhost:8000/api/v1/market-data/analytics/cache-performance

# 백테스트 성과 분석 (DuckDB 기반)
curl http://localhost:8000/api/v1/backtests/analytics/performance-stats
...
| ---------------------- | ---------------------------------- | -------------------------------- | ------- |
| **ServiceFactory**     | DatabaseManager, 모든 서비스       | 모든 API 엔드포인트              | ✅ 정상 |
| **DatabaseManager**    | DuckDB                             | ServiceFactory                   | ✅ 정상 |
| **MarketDataService**  | DatabaseManager, Alpha Vantage API | DataPipeline, IntegratedExecutor | ✅ 정상 |
| **StrategyService**    | MongoDB                            | IntegratedExecutor, Strategy API | ✅ 정상 |
| **BacktestService**    | DatabaseManager, MongoDB           | Backtest API, IntegratedExecutor | ✅ 정상 |
...
| `/market-data` | 8             | 시장 데이터 관리 + DuckDB 분석 | MarketDataService |
| `/strategies`  | 8             | 전략 관리                      | StrategyService   |
| `/backtests`   | 12            | 백테스트 실행 + DuckDB 분석    | BacktestService   |
| `/templates`   | 6             | 전략 템플릿 관리               | StrategyService   |
